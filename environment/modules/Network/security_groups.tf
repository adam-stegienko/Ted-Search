resource "aws_security_group" "adam-sg" {
  ...
  ingress {
    description = var.ingress_three["description"]
    from_port   = var.ingress_three["from_port"]
    to_port     = var.ingress_three["to_port"]
    protocol    = var.ingress_three["protocol"]
    cidr_blocks = [var.ingress_three["cidr_blocks"]]
  }
